...
public class OrdersBean {
	...
	public ArrayList<BookBean> getBooks()
	{
		...
		{
			list.add(order_details.get(i).getBook());
		}
		return list;
	}
	...
	public int getQuantityInOrder(int bookId)
	{
		int total = 0;
		for (int i = 0; i < order_details.size(); i++) 
		{
			if (order_details.get(i).getBook().getId() == bookId)
				total = order_details.get(i).getQuantity();
		}
		return total;
	}
